...
{				// Rx thread
  ...
  {				// Rx websocket packets
    while (1)
      {
	unsigned char head[14], hptr = 0, hlen = 2;
	ssize_t len = 0;
	while (hptr < hlen)
	  {
	    if (w->ss)
	...
	if (len == 126)
	  ...
	  len =
	    ...
	    ((unsigned long long) head[5] << 32) + ((unsigned long long) head[6] << 24) + ((unsigned long long) head[7] << 16) + ((unsigned long long) head[8] << 8) + ((unsigned long long) head[9]);
